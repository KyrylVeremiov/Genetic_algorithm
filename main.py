...
def int_to_bytes(P):
    A = np.array(
        [['+' + bin(P[j][i])[2:] if P[j][i] >= 0 else '-' + bin(P[j][i])[3:] for i in range(P.shape[1])] for j in
         range(P.shape[0])])
    return np.array(
        [[A[j][i][0] + str('0' * (n_bits - len(A[j][i][1:]))) + A[j][i][1:] for i in range(A.shape[1])] for j in
         range(A.shape[0])])
...
def fitness(P):
    X = bytes_to_int(P)
    return np.array([(P[i], f(X[i])) for i in range(X.shape[0])])
...
def get_population(P):
    ...
    X = bytes_to_int(newP)
    return np.array([(X[i], f(X[i])) for i in range(X.shape[0])])
...
def make_initial_population(count, a, b):
    ...
    return int_to_bytes(np.random.randint(a, b, (count, D)))
...
def crossover(p1, p2):
    ...
    for i in range(D):
        ...
        g2 = p2[0][i]
        # print(g1, g2)
        ng1i = g1[:int(n_bits / 2) + 1] + g2[int(n_bits / 2) + 1:]
        ng2i = g2[:int(n_bits / 2) + 1] + g1[int(n_bits / 2) + 1:]
        c1.append(ng1i)
        c2.append(ng2i)
...
def crossover_population(P, count_of_children):
    ...
    while len(new_children) < count_of_children:
        new_children += crossover(P[np.random.randint(0, P.shape[0])], P[np.random.randint(0, P.shape[0])])
...
def mutate(p):
    ...
    for i in range(D):
        g = p[i]
        I = np.random.randint(1, n_bits+1)
        # print(I)
...
def mutate_population(P):
    ...
    for i in range(P.shape[0]):
        new_generation += mutate(P[i])
    return np.array(new_generation)

def show_results(fP):
    tmp_population = get_population(fP[:,0])
    print()
    print('X: ')
    ...
    print('Total score:')
    print(get_population_condition(tmp_population))
    print('--------------------------------------')
    print()
...
def solve(F,show_calculations):
    f=F
    # %%
    population = make_initial_population(size_of_population, A, B)
    # %%
    ...
    #%%
    for i in range(t):
        new_children = crossover_population(fit_population, size_of_population)
        new_generation=mutate_population(new_children)
        new_fit_generaton=fitness(new_generation)
        fit_population = tournament_selection(np.concatenate((new_fit_generaton,fit_population)), tournament_size, size_of_population)
    ...
    final_population=get_population(fit_population[:,0])
